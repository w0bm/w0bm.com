        ...
        <div class="wrapper">
			...
			<div class="text-center" style="position: unset;">
				...
				<span class="videoinfo">
						...
						uploaded by <span style="color: rgb(233, 233, 233);"><a href="{{ url('user/' . $video->user->username) }}">{{ $video->user->username }} @if($video->user->is('Moderator'))<span class="fa fa-bolt anim"></span>@elseif($video->user->is('Editor'))<span class="fa fa-shield anim"></span>@elseif($video->user->is('flinny'))<img class="flinnysmall" src="{{ asset('wizard.png') }}"></img>@elseif($video->user->is('brown'))<img class="rainsmall" src="{{ asset('watermelon.png') }}"></img>@elseif($video->user->is('dank')) <img class="danksmall" src="{{ asset('weed.png') }}"></img>@elseif($video->user->is('gay'))<img class="gaysmall" src="{{ asset('gaypride.svg') }}"></img>@elseif($video->user->is('duke'))<img class="dukesmall" src="{{ asset('duke.png') }}"></img>@elseif($video->user->is('alugay'))<img class="americagaysmall" src="{{ asset('RainbowAmerica.svg') }}"></img>@elseif($video->user->is('reis')) <img class="onigiri" src="{{ asset('onigiri.gif') }}"></img>@elseif($video->user->is('bio')) <img class="biosmall" src="{{ asset('bio.png') }}"></img>@elseif($video->user->is('patoy')) <img class="patoyklein" src={{asset('patoy.png')}}></img>@endif</a></span> <time class="timeago" data-toggle="tooltip" data-placement="bottom" datetime="{{$video->created_at}}+0000" title="{{$video->created_at}} UTC">{{ $video->created_at->diffForHumans() }}</time>@if(auth()->check() && auth()->user()->can('delete_video')) <a data-confirm="Do you really want to delete this video?" class="" href="{{url($video->id . '/delete')}}"><i style="color:red;" class="fa fa-times" aria-hidden="true"></i></a>@endif @if(auth()->check() && (auth()->user()->can('edit_video') || auth()->user()->id == $video->user_id))<a href="#" data-toggle="modal" data-target="#webmeditmodal"><i style="color:#2ada19;" class="fa fa-pencil-square"></i></a>@endif
				</span>
			</div>
		</div>
	</div>
