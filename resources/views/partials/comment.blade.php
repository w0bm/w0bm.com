<div class="panel @if($del = !is_null($comment->deleted_at)) panel-danger @else panel-default @endif">
    ...
    <div class="panel-footer">by <a href="/user/{{$comment->user->username}}">{{$comment->user->username}}</a> <small><span data-toggle="tooltip" data-placement="right" title="{{$comment->created_at->timezone('Europe/Berlin')->format('d.m.Y H:i')}}">{{$comment->created_at->diffForHumans()}}</span></small>
        @if($mod)
           @if($del)
